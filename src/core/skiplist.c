...
** iterator
*****************************************/
static int it_has_next(skiter_t *it)
{
    if (it->pri->cur_pos == NULL 
        || it->pri->cur_pos->lvs[0].fw == NULL) return 0;

    return 1;
}

static int it_next(skiter_t *it, void **out_data)
{
    it->pri->cur_pos = it->pri->cur_pos->lvs[0].fw;

    if (out_data) *out_data = it->pri->cur_pos->data;

    return 0;
}

static int it_current(skiter_t *it, void **out_data)
{
    if (it->pri->cur_pos == NULL || it->pri->cur_pos->data == NULL) return 0;
...
static void  it_destroy(skiter_t *it)
{
    RWUNLOCK(&it->container->lock);
    del_obj(it);
}

static int it_init(skiter_t *it)
{
    /* unlock in destroy */
    RWLOCK_READ(&it->container->lock);

    it->pri->cur_pos = it->container->head;
    return 0;
}

skiter_t *skiter_create(pool_t *mpool)
{
    skiter_t *it = new_obj(mpool, sizeof(*it) + sizeof(struct skiter_pri));
    if (it == NULL) return NULL;

    it->pri = (struct skiter_pri *)((char *)it + sizeof(*it));
    it->init = it_init;
    it->has_next = it_has_next;
    it->next = it_next;
    it->current = it_current;
