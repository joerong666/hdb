...
struct thpool* thpool_init(int thread_num, int queue_max_num)
{
    struct thpool *pool = NULL;
    do 
    {
        pool = malloc(sizeof(struct thpool));
        if (NULL == pool)
        {
            printf("failed to malloc thpool!\n");
            break;
        }
        ...
        {
            pthread_create(&(pool->pthreads[i]), NULL, thpool_function, (void *)pool);
        }
...
int thpool_add_job(struct thpool* pool, void* (*callback_function)(void *arg), void *arg)
{
    assert(pool != NULL);
...
void* thpool_function(void* arg)
{
    struct thpool *pool = (struct thpool*)arg;
    ...
    while (1)  //死循环
    {
        ...
        if (pool->queue_cur_num == 0)
        {
            pthread_cond_signal(&(pool->queue_empty));        //队列为空，就可以通知thpool_destroy函数，销毁线程函数
        }
        if (pool->queue_cur_num == pool->queue_max_num - 1)
        {
            pthread_cond_broadcast(&(pool->queue_not_full));  //队列非满，就可以通知thpool_add_job函数，添加新任务
        }
        pthread_mutex_unlock(&(pool->mutex));
...
int thpool_destroy(struct thpool *pool)
{
    assert(pool != NULL);
    pthread_mutex_lock(&(pool->mutex));
    ...
    pthread_cond_broadcast(&(pool->queue_not_full));   //唤醒添加任务的thpool_add_job函数
    int i;
    for (i = 0; i < pool->thread_num; ++i)
    {
...
{
    char *p = (char*) arg;
    printf("thpool callback fuction : %s.\n", p);
    sleep(1);
}
